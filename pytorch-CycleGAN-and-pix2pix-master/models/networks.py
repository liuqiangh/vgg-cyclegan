...
def define_D(input_nc, which_model_netD,
            norm='batch', use_sigmoid=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    netD = None
    norm_layer = get_norm_layer(norm_type=norm)

    if which_model_netD == 'basic':
        netD = NLayerDiscriminator(input_nc, norm_layer=norm_layer,use_sigmoid=False)
    # elif which_model_netD == 'n_layers':
    #     netD = NLayerDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer, use_sigmoid=use_sigmoid, use_dropout=False)
    # elif which_model_netD == 'pixel':
    #     netD = PixelDiscriminator(input_nc, ndf, norm_layer=norm_layer, use_sigmoid=use_sigmoid, use_dropout=False)
    # else:
...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target_is_real):
        if target_is_real:
            target_tensor = self.real_label.type(torch.cuda.LongTensor)
            #target_tensor = self.real_label
        else:
            target_tensor = self.fake_label.type(torch.cuda.LongTensor)
            #target_tensor = self.fake_label
        #return target_tensor.expand_as(input)
        input_size = list(input.size())
        #print(input_size)
        return target_tensor.expand(input_size[0])
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, norm_layer=nn.BatchNorm2d,use_sigmoid=False):
        ...
        sequence = [
            ...
            nn.ReLU(),
            norm_layer(32),
            nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1, bias=use_bias),
            nn.ReLU(),
        ...
        sequence += [
            norm_layer(512),
            nn.Conv2d(512, 1024, kernel_size=3, stride=1, padding=0, bias=use_bias),
            nn.ReLU(),
            #norm_layer(1024),
            nn.Conv2d(1024, 512, kernel_size=1, stride=1, padding=0, bias=use_bias),
            nn.ReLU(),
            #norm_layer(512),
            nn.Conv2d(512, 2, kernel_size=1, stride=1, padding=0, bias=use_bias),
        ]
        #sequence += [Reshape(2)]
        
        if use_sigmoid:
            sequence += [nn.Sigmoid(), Reshape(2)]
        else:
            sequence += [Reshape(2)]
