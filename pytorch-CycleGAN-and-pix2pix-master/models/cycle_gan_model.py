...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD_A = networks.define_D(opt.output_nc, opt.which_model_netD, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_B = networks.define_D(opt.input_nc, opt.which_model_netD, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            #self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()),
                                                #lr=opt.lr, betas=(opt.beta1, 0.999), weight_decay = 0.0003)
            self.optimizers = []
